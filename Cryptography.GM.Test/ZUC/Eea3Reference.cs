...
public class Eea3Reference
{
    [Theory]
    [InlineData(
        new byte[] { 0x17, 0x3d, 0x14, 0xba, 0x50, 0x03, 0x73, 0x1d, 0x7a, 0x60, 0x04, 0x94, 0x70, 0xf0, 0x0a, 0x29 },
        0x66035492, 0xf, 0, 193,
        new byte[] {
    ...
    [InlineData(
        new byte[] { 0xe5, 0xbd, 0x3e, 0xa0, 0xeb, 0x55, 0xad, 0xe8, 0x66, 0xc6, 0xac, 0x58, 0xbd, 0x54, 0x30, 0x2a },
        0x56823, 0x18, 1, 800,
        new byte[] {
    ...
    [InlineData(
        new byte[] { 0xd4, 0x55, 0x2a, 0x8f, 0xd6, 0xe6, 0x1c, 0xc8, 0x1a, 0x20, 0x09, 0x14, 0x1a, 0x29, 0xc1, 0x0b },
        0x76452ec1u, 2, 1, 1570,
        new byte[] {
    ...
    [InlineData(
        new byte[] { 0xdb, 0x84, 0xb4, 0xfb, 0xcc, 0xda, 0x56, 0x3b, 0x66, 0x22, 0x7b, 0xfe, 0x45, 0x6f, 0x0f, 0x77 },
        0xe4850fe1u, 0x10, 1, 2798,
        new byte[] {
    ...
    [InlineData(
        new byte[] { 0xe1, 0x3f, 0xed, 0x21, 0xb4, 0x6e, 0x4e, 0x7e, 0xc3, 0x12, 0x53, 0xb2, 0xbb, 0x17, 0xb3, 0xe0 },
        0x2738cdaa, 0x1a, 0, 4019,
        new byte[] {
    ...
    public void TestVector(byte[] sk, uint count, byte bearer, byte direction, int bits, byte[] plaintext, byte[] ciphertext)
    {
        var iv = new byte[16];
        iv[0] = (byte)(count >> 24);
        iv[1] = (byte)(count >> 16);
        iv[2] = (byte)(count >> 8);
        iv[3] = (byte)count;
        iv[4] = (byte)((bearer << 3) | (direction << 2));
        iv[5] = iv[6] = iv[7] = 0;
    ...
    {
        var rng = new Random();
        var versions = Enum.GetValues(typeof(ZucVersion));
        while (n-- > 0) {
            var sk = new byte[32];
            var iv = new byte[23];
            ...
            rng.NextBytes(iv);
            rng.NextBytes(data);
            yield return new[] { sk, iv, version, data };
        }
    }
}
